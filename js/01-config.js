...
// ──────────────────────────────────────────────────────────────────────────
var CURRENT_VERSION = "10.232";
var VERSION_HISTORY = [
  {v:"10.232", date:"Aug 2026", headline:"FIX (mobile) — the Save / Import / Export / New Deal actions are now IN THE MENU on your phone. On a phone the top bar can only show a couple of the round emoji buttons before the rest scroll off the right edge, and the scrollbar is hidden — so those key actions looked like they'd vanished (the ‘I've lost the header’ problem). They now ALSO live at the top of the ☰ menu under ‘Deal actions’: tap the hamburger (top-left) and you get the full set — 📎 Upload, 🎙 Talk, 🧠 Analyse, 💾 Save, 🔀 Save As, ⬆ Import, ⬇ Export, ✕ New Deal — as proper labelled buttons that always fit, each closing the menu when tapped. The top-bar strip is unchanged and desktop is unaffected. No engine change."},
  {v:"10.231", date:"Aug 2026", headline:"FIX — the Save / Import / New Deal buttons no longer hide off the edge of the top bar. The top action bar scrolls sideways when there are more buttons than fit, but the scrollbar is hidden, so on a narrower window the key deal actions (Save Deal, Save As, Export, Import, New Deal) — which sat at the far right — looked like they'd vanished. They're now PRIORITISED to the front of the bar (along with Upload, Talk and Analyse), so the eight actions you use most are always visible on the left; only the secondary buttons (Flowchart, History, Report, Sign out) scroll off the right if the window's tight. No other change."},
  ...
  {v:"10.226", date:"Aug 2026", headline:"Ronald now picks the most NATURAL voice your device has, automatically. The voice chooser was retuned to strongly prefer the genuinely human speech engines — Google's Natural / WaveNet / Studio voices (Chrome/Edge, Android), Apple's Enhanced / Premium Siri voices (iPhone/Mac), and Microsoft's Neural / Online voices — and to push the tinny robotic fallbacks (compact / eloquence / novelty voices) to the very bottom, so Ronald sounds as human as the device allows without you touching a setting. TIP for the best quality: the top-tier voices are an optional download — on iPhone/iPad, Settings → Accessibility → Spoken Content → Voices → English → download a British ‘Enhanced/Premium’ voice (e.g. Daniel); on Chrome/Edge desktop the ‘Google UK English’ / ‘Natural’ voices are built in. You can still hand-pick any installed voice from ‘Ronald's voice’. (A truly studio-grade custom/cloned voice would need a neural TTS service — a separate paid step we've left out for now.) No engine change."},
  {v:"10.225", date:"Aug 2026", headline:"Ronald's web look-ups now run through the CLAUDE API YOU ALREADY HAVE — no separate search key. Ask him things like “look up next year's RPI forecast so we can allow for higher build costs”, “what's the latest on nutrient neutrality”, “search the new NPPF changes”, and Claude's built-in web search does the browsing and hands back a UK-developer answer, billed to your existing Anthropic key. Deploy is one paste: docs/ronald-web-search.gs upgraded to use the same ANTHROPIC_KEY that already powers Landform (plus add the ‘web_search’ route line) — no Google/Bing account needed. Until it's deployed he still answers from knowledge with a ‘verify the live source’ caveat, and every confirmed finding is saved into his 🧠 memory. No engine change."},
  {v:"10.224", date:"Aug 2026", headline:"Ronald can now LOOK THINGS UP on the live web — the second half of learning. Say “look up the latest BNG rules”, “search for the new NPPF changes”, “what's the latest on nutrient neutrality” and he fetches current results, tells you the position and what it means for your appraisals, and REMEMBERS the finding (it lands in his 🧠 memory, tagged as a web check). Honest by design: this needs a one-time backend step (docs/ronald-web-search.gs — add a search API key to your Apps Script; free Google tier is fine). Until that's deployed he answers from his own knowledge and tells you to verify anything recent against the official source, rather than pretending he browsed. Only explicit research phrases (“look up…”, “search for…”, “find out about…”) trigger a web search — normal conversation doesn't. No engine change."},
  {v:"10.223", date:"Aug 2026", headline:"Ronald now has a MEMORY — he learns as you go. Tell him “remember we always target a 20% margin”, “remember BNG is 12.5% in our area”, “note that this vendor prefers an option” and he keeps it for good, feeding it into EVERY future conversation so his advice reflects how you actually work (your standing facts override generic assumptions). Say “what do you remember” to hear/see the list, or open the 🧠 button in his panel to read, add or delete items by hand; “forget everything” clears it. Memory is per-account and works on the device now; deploy docs/ronald-memory-sync.gs (a ~2-minute backend add, same as the pipeline sync) to carry it across all your devices. This is the first half of learning; the live web-check for brand-new regulations follows next. No engine change."},
  {v:"10.222", date:"Aug 2026", headline:"Ronald is now a FLOATING ASSISTANT that follows you around Landform and takes commands. Tap ‘🎙 Talk’ (top bar) and he opens as a small panel in the corner that STAYS with you as you move between pages — he keeps listening and talking the whole time, like a virtual assistant on a call. And he takes spoken COMMANDS, acted on instantly (no wait): say ‘show me the one-page appraisal and print it out’ and he opens it (and the print dialog); ‘open the dashboard’, ‘take me to the exit strategy’, ‘go to reports’ and he navigates there while still talking; ‘build the scheme’ / ‘build it’ and he builds. He understands ~25 pages by name (dashboard, portfolio, quick appraisal, board proposal, exit, house mix, BTR/PBSA block, capitalisation, grants, planning, financials, due diligence, risks, land valuation, mixed-use, Placona, approach landowner, and more). Ordinary conversation isn't hijacked — a command needs a clear instruction (‘show me…’, ‘open…’, ‘print…’, ‘build…’). The old full-screen voice page is now just a launcher for the floating panel. No engine change."},
  {v:"10.221", date:"Aug 2026", headline:"SPEED — Ronald replies noticeably quicker. Each turn used to make two AI calls back-to-back (first extract the deal figures, then compose the reply), so you waited for both. Now the reply and the figure-extraction run at the SAME TIME: Ronald answers as soon as his reply is ready, using the figures already on screen (which keep firming up as you talk), and the live numbers refresh in the background for the panel and his next answer. Same answers, roughly half the wait. No engine change."},
  {v:"10.220", date:"Aug 2026", headline:"Ronald now READS THE PLACONA AGENT before he builds. The land-search agent already captures a lot per site — estimated units, planning status, LPA, guide price, constraints, its own score, the recommended action, plus your CRM contact and follow-ups — but Ronald was ignoring it and starting from a blank slate. Now, when you open a conversation on a site that came from Placona, Ronald (1) pulls all of that detail in, (2) seeds the brief and runs the engine so the LIVE FIGURES (GDV, residual land, margin) are on screen from the first second, and (3) OPENS BY REPORTING IT BACK — ‘Right, I've pulled up [site] from Placona: ~180 homes on 12.5 acres, allocated, guide £4.5m; I've run the numbers — GDV ~£70m, residual land ~£16.5m, ~34% margin at the guide. Shall we work these up, or change the approach?’ A ‘📋 Read from the Placona agent’ panel shows exactly what he picked up, and every captured fact now flows into the build (so the scheme isn't re-guessed from scratch). Nothing from Placona? He opens as before. No engine change."},
  {v:"10.219", date:"Aug 2026", headline:"FIX — Ronald now speaks on iPhone/iPad. iOS Safari only allows a page to speak if the speech starts INSIDE your tap; Ronald speaks a beat later (on a timer, and again after the AI replies), which iOS was silently blocking — so it heard you and showed his reply, but stayed silent. Now every Start/tap ‘unlocks’ speech with a silent primer, so the opener and all of Ronald's replies play aloud for the rest of the session. No change on desktop. No engine change."},
  {v:"10.218", date:"Aug 2026", headline:"FIX — hands-free Ronald no longer freezes after the first question. Two bugs were stalling the loop: (1) some browsers (Chrome especially) don't fire the ‘finished speaking’ event reliably — and because the mic only reopened after that event, it never reopened, leaving Ronald stuck on ‘speaking…’ while you talked to a dead mic. Ronald now keeps a live reference to each utterance (Chrome was garbage-collecting it) and runs a watchdog that reopens the mic the moment speech actually stops, so the turn-taking always advances. (2) The ‘listen again’ and ‘send’ callbacks were capturing the very first render's state, so each turn wiped the thread and sent Ronald no history — now they always call the current handlers, so your conversation accumulates properly and Ronald replies with full context. Just hit Start and talk. No engine change."},
  {v:"10.217", date:"Aug 2026", headline:"Talking to Ronald is now HANDS-FREE — just hit Start and talk, no Send button. When you start a conversation, Ronald greets you, then listens; when you stop speaking he sends your turn automatically, replies out loud, and listens again — a natural back-and-forth like a phone call. A status line shows what he's doing (‘🎤 Listening — just talk’, ‘Ronald is thinking…’, ‘Ronald is speaking…’) and a ‘⏸ Pause’ button stops the mic whenever you want (‘🎤 Start talking’ resumes). Typing is still there as a fallback (a smaller box + Send), and leaving the screen now properly stops Ronald talking/listening. No engine change."},
  {v:"10.216", date:"Aug 2026", headline:"Meet RONALD — the voice operator now has a name, a more natural voice, and remembers your conversation. (1) The operator introduces himself as Ronald and is Ronald throughout (the ‘🎙 Talk’ button, the chat, the stage ‘Ronald (Voice)’). (2) NATURAL VOICE: instead of the browser's robotic default, Ronald now picks the most natural voice available (preferring en-GB and the ‘Natural’ / Google / Siri-grade voices, male-leaning), and there's a ‘Ronald's voice’ dropdown so you can choose any installed voice — it previews as you pick. Voice quality is capped by what your browser/device has installed; the Google ‘Natural’ voices (Chrome/Edge desktop) and the Apple voices (iPhone/Mac) are far less synthetic — for a truly human, studio-grade voice we'd wire a neural text-to-speech service, which is a further step. (3) The CONVERSATION now PERSISTS — switch tabs, or let the app auto-refresh, and your chat with Ronald (and the live figures) is still there; a ‘↺ New’ button starts a fresh one, and it clears once you build the scheme. No engine change."},
  {v:"10.215", date:"Aug 2026", headline:"A ‘🎙 Talk’ button now sits in the TOP BAR, always visible, so you can talk to Landform from anywhere in one tap — it opens the Voice Operator (start a conversation or a quick guided interview). Before, the voice mode was only reachable as a stage in the ‘0. Start’ section of the left menu, which was easy to miss; now the button is right beside ‘Analyse Deal’ on every screen. No other change."},
  {v:"10.214", date:"Aug 2026", headline:"The Landowner Identifier's title lookup is now wired to LANDINSIGHT (LandTech). Press ‘📜 LandInsight — look up the title & owner’ and, once your backend carries the key, it fetches the registered proprietor and their address for the site (or a title number) straight from LandInsight and lets you record the owner into the Approach-Landowner stage with the title number captured as the source. Setup is a one-time backend step — get an API key from your LandTech / LandInsight account, add LANDTECH_API_KEY (and LANDTECH_API_BASE) to your Apps Script properties, and paste the ‘land_registry_lookup’ handler from docs/landowner-identify.gs (upgraded to a LandInsight integration: Bearer auth, the ownership endpoint, and a response mapper that reads the proprietor / address / title fields — confirm the exact endpoint path and field names against your LandTech API reference, which ships with your credentials, and adjust the two marked spots if they differ). Until it's deployed the button explains what's needed and you use the official HM Land Registry title link. Company lookups stay free via Companies House. No engine change."},
  {v:"10.213", date:"Aug 2026", headline:"NEW: ‘🕵 Landowner Identifier’ (1. Find) — when you can't find who owns the land by other means, this helps you identify them. Enter the site and whatever you know (agent hints, a farm/estate name, planning history) and it (1) runs AI DESK RESEARCH to suggest the most likely owner, its type and confidence, with the best routes to confirm ranked best-first; (2) links you straight to the OFFICIAL sources — the HM Land Registry title register (the definitive proprietor name + address, ~£3), Companies House (free), the free CCOD/OCOD corporate/overseas-ownership datasets, and the INSPIRE parcel map; (3) can call a wired ownership lookup from your backend — Companies House is FREE (just an API key) and confirms a company owner + registered office; the definitive title lookup needs an HMLR Business Gateway account or an aggregator (Searchland / LandInsight / Nimbus), with a ready starter in docs/landowner-identify.gs; and (4) records the confirmed owner and pushes it straight into the Approach-Landowner stage, so identify → contact is one flow. Honest by design: there is NO free ‘who owns this postcode’ API — the title register is the authority; not all land is registered; use the data only for a genuine business enquiry. New screen screen-LandownerIdentifier.js. No engine change."},
  ...
  {v:"10.207", date:"Aug 2026", headline:"The Voice Operator now quotes LIVE FIGURES mid-conversation. As you talk, Landform continuously updates a running brief from what's been said and runs the SAME engine as the rest of the tool on it — so when you change something the operator answers with the actual number: ‘at 30% affordable the residual's about £4.2m’, ‘on a forward-fund at that yield it's nearer £X’. Each turn it re-reads the conversation → deal brief → engine (residual land value, GDV, developer margin, and the land supported under each exit — plot sales / HA bulk / forward-fund), feeds those into the operator's context so it can only quote the REAL engine numbers (not invented ones), and shows them in a live ‘Live figures’ strip above the chat that updates as you go. Figures appear as soon as it knows roughly the homes/acres and location, and firm up as you add detail. Uses buildDealFromBrief + calcDealMetrics + dealExit + afterLandMargin — identical to a built deal — so the mid-chat numbers match what you get when you build. No engine change."},
  {v:"10.206", date:"Aug 2026", headline:"You can now have a REAL CONVERSATION with the Voice Operator — not just answer set questions. A new ‘💬 Start a conversation’ mode lets you think aloud, ask it anything, and throw in points and requests; the operator TALKS BACK — answers your questions with practical UK planning / development / finance knowledge, gives a candid view on your thinking (names the risk and the upside), and draws out what it needs, a point or two at a time, out loud. Everything you raise is folded into the transcript, so when you say ‘build it’ every thought and request is built into the scheme (then priced and due-diligenced start to finish, as before). Speak (🎤) or type each turn; replies are read aloud (toggle-able). The original fixed ‘guided interview’ is still there as a quick led option. Powered by the same AI backend as the rest of the tool; graceful fallback to typing where speech isn't supported. New conversation loop in screen-VoiceOperator.js (shared build pipeline runBuildFromSource, so a conversation and a guided interview build identically). No engine change."},
  {v:"10.205", date:"Aug 2026", headline:"The Voice Operator now takes the scheme START TO FINISH on its own. After the spoken interview it doesn't just build the shell — it runs the FULL Keystone completion automatically: it prices and optimises the house mix from current market listings, then adds the due diligence it can (Planning, Exit strategy, Grants and Constraints) via the same journey fillers Keystone uses, all through the one tested engine. You end at a finished, priced, DD-populated deal with a single ‘📑 Generate the reports →’ step to produce the board / marketing / stakeholder / investor documents ready to approach the land agent, the owner and the planners. So the flow is now: type or Placona-load the site → 🎙 hit Start and answer out loud → Landform builds, prices, DDs and readies the whole scheme. Uses the shared applyMarketPricesAndOptimise() and KEYSTONE_JOURNEY_FILLERS, so the result is identical to completing it by hand in Keystone. Follows v10.204. No engine change."},
  {v:"10.204", date:"Aug 2026", headline:"NEW: a VOICE OPERATOR — talk to Landform. Once a site is in the tool (typed in or loaded from a Placona search), a new ‘🎙 Voice Operator’ stage (0. Start) lets you hit Start and have Landform SPEAK a guided interview aloud — your intention for the land, the current planning status, constraints, ownership, the land agent / vendor, the scheme, the exit, affordable % and any notes — and you answer OUT LOUD. Your spoken answers are captured live (Web Speech API), shown for you to edit, then assembled into a brief and handed to KEYSTONE, which builds the whole scheme through the one tested engine and drops you at ‘Complete with AI & reports’ — ready to price the mix, add the due diligence it can, and print the marketing, stakeholder and approach documents. It reads the site details you’ve already entered so it doesn’t re-ask them, and it uses the SAME AI extraction and build path as typing into Keystone, so the numbers are identical. Graceful fallback: a browser without speech support runs the exact same guided question-and-answer, typed. New screen screen-VoiceOperator.js (a self-contained component using SpeechRecognition to listen and SpeechSynthesis to ask). No engine change — a new front door to the existing Keystone build."},
  ...
  {v:"10.190", date:"Jul 2026", headline:"Fix: on the Placona Pipeline / CRM, the stage dropdown and the contact / next-action / due-date boxes were frozen — you couldn't type or change them (only dictating into the activity box worked). Cause: the CRM's save helper called setData(), but the Placona screen is passed up() (not setData), so every one of those handlers threw and nothing saved. Rewired to persist through up() like the rest of the screen — stage, contact details, next action and due date all save now, on both the site detail card and the Pipeline list/board. (The activity log kept working because its draft is local until you press Add — which was also affected and is fixed too.) No data change."},
  {v:"10.189", date:"Jul 2026", headline:"The Placona Notes tab is now a small CRM — a ‘📋 Pipeline’ for the land you're chasing. Each opportunity moves through pipeline STAGES (New → Enquiry made → Viewing → Offer made → Under offer → Legals → Completed / Lost), and gets: CONTACT details for the agent / vendor (name, company, phone, email — with tap-to-call and email links); a dated ACTIVITY LOG (each interaction timestamped, newest first — type or 🎤 dictate an entry, then Add); and a NEXT ACTION + due date, with overdue actions flagged in red (and an ‘⏰ N overdue’ count at the top). Two views via a toggle: a ▦ BOARD (kanban) with a column per stage — drag-free, move a card between stages with its dropdown — and a ☰ LIST of expandable cards. Filter the pipeline by stage or by area (county). The same CRM card sits at the top of each site's detail view, and the inbox cards show the stage badge. Your earlier v10.188 notes/statuses carry over (old ‘Viewed’ → ‘Viewing’, ‘Passed’ → ‘Lost’). All client-side per this device for now (a team-shared version needs a backend field); stored in the Placona workspace, keyed by postcode + name so it survives re-running the search. No engine change."},
  {v:"10.188", date:"Jul 2026", headline:"NEW ‘📝 Notes’ tab on the Placona land finder — log notes and enquiries on the land you've viewed. Each site gets a free-text notes box (who you spoke to, asking price discussed, viewing arranged, next steps) and an ENQUIRY STATUS — Not contacted / Enquiry made / Viewed / Offer / Passed — so you can track what you've actually chased. Notes can be DICTATED: a ‘🎤 Dictate’ button (speech-to-text) appends what you say, and the box also takes your phone/desktop keyboard's own mic. The Notes tab lists every site in your inbox (the ones you've engaged with rise to the top) with its status badge, score and an ‘Open →’ into the full site; the same notes box also sits at the top of each site's detail view so you can jot an enquiry while you're looking at the land; and the inbox cards now show a coloured status badge (e.g. ‘Enquiry made’) so contacted land stands out at a glance. Notes are keyed by the site's postcode + name, so they survive re-running the search. Stored in the Placona workspace; no engine change."},
  {v:"10.187", date:"Jul 2026", headline:"Two fixes to the Placona ‘Find Land Opportunities’ inbox. (1) NEW area filter: the inbox can hold a lot of land across the country, so there's now a ‘📍 Area’ dropdown that narrows it to a county — the options are the counties actually present in your inbox, each with a site count (e.g. ‘Kent (12)’), plus ‘All areas’. It works alongside the score shortlist; changing it re-fits the map to that area, and a ‘no sites match’ card with a Reset offers a way back if a filter hides everything. (2) FIX: the opportunities map no longer zooms back out to the whole country when you open a site and return. It kept re-creating itself at the default UK-wide view and re-fitting to every pin on each remount; now it remembers the centre & zoom you left it at and only auto-fits when the SITE SET actually changes (first load, or an area/score filter change) — so picking a site and coming back keeps your place. No engine change."},
  {v:"10.186", date:"Jul 2026", headline:"Developer margin now shows the actual £ profit beside the percentage everywhere it appears on the appraisals — a slash then the sterling figure, e.g. ‘17.5% / £12.3m’ — so the margin isn't an abstract percentage; you see the money it represents. Applied to the Quick Appraisal margin stat, the one-page appraisal PDF margin KPI, the Board Proposal ‘Developer profit’ line and headline ‘Dev margin’ tile, and the SFH residual summary. The £ shown is the ACTUAL profit at the entered guide/land price where one is set, or the target profit £ otherwise (matching the % shown). Any ‘with AHP grant’ note is unchanged and still sits alongside. No engine-value change — the figures were already computed; this surfaces the £ next to the %."},
  {v:"10.185", date:"Jul 2026", headline:"The one-page appraisal PDF now opens with an EXIT SNAPSHOT banner, so a team member sees at a glance WHICH exit it's appraising and its headline economics — the same PDF button produces the right snapshot for the committed exit. On a FORWARD-FUND deal it reads ‘📊 Forward-fund appraisal — rented investment let and sold to a fund / HA at a 4.75% net initial yield. Net rent £X/yr → the fund pays £Y → supports land of £Z across N homes.’ On a build-to-sell deal it reads ‘📊 Build-to-sell appraisal — N homes sold on the open market. GDV £X → supports land of £Y at 17.5% profit.’ (and a bulk-sale-to-HA variant). It adapts to the exit committed on the Exit Strategy stage (dealExit), sitting right under the site title above the KPIs, so no one has to read the whole page to know the basis. The verdict beneath was already exit-aware; this makes the document self-labelling. Shared one-pager, so it prints from both the Quick Appraisal and the Board Proposal. No engine-value change."},
  {v:"10.184", date:"Jul 2026", headline:"The Quick Appraisal now reflects the EXIT you've committed on the Exit Strategy stage — so if you've chosen an HA / institutional FORWARD-FUND (the finished rented scheme sold to a fund/HA at a net initial yield), the whole verdict is on THAT basis, not build-to-sell. Before, the headline ‘Worth to us’ followed the chosen exit but the stack verdict, margin and headroom were still computed on the open-market plot-sales residual — so a forward-fund deal read against the wrong numbers. Now, when a forward-fund exit is committed: the land value is the capitalised residual (rent ÷ your chosen net yield), the margin is the developer margin on what the fund pays, the headroom tests the asking price against that, and the verdict banner is badged with the exit, yield and net rent (‘Exit: capitalised rental investment · 4.75% net yield · rent £X/yr’). The ‘What to change to make it stack’ box becomes exit-aware too: on a forward-fund the levers are the YIELD and the RENTS (a keener yield or higher rents lift what the fund pays and the land value) — it points you to set both on the Capitalisation stage — rather than the sale-£/sqft levers used for build-to-sell. Build-to-sell and the other exit routes are still there under ‘Show full appraisal’, but the default view reflects only your committed exit. No engine-value change — it reuses the same forward-fund capitalisation the Capitalisation stage and the exit-routes card already compute. (Set the exit to ‘Institutional forward-fund’ on the Exit Strategy stage, and the rents/yield on Capitalisation, for it to show.)"},
  {v:"10.183", date:"Jul 2026", headline:"Quick Appraisal is now verdict-first, and the one-page appraisal shows grant intelligence on every deal. (1) The Quick Appraisal LEADS with the answer — a big ‘does it stack?’ verdict, a compact RLV / asking / headroom / margin strip, and a ‘🔧 What to change to make it stack’ box (the sale-price / build-cost / affordable-% / profit levers that reach a 15% margin) — with the one-page-PDF and refine-mix buttons right there. The full appraisal breakdown, profit-sensitivity table, exit routes and forward-fund exit are unchanged but tucked behind a ‘Show full appraisal, sensitivity & exits ▾’ toggle (collapsed by default), so the page answers ‘stack or not, and what to change’ at a glance instead of being a wall of numbers. Reported: ‘too much info’. No figure changes — layout only. (2) The one-page appraisal (shared by Quick Appraisal AND the Board Proposal, so it prints on every deal) now carries a compact AHP GRANT line whenever the scheme has affordable housing: the indicative grant per affordable home (~£45k) × the affordable-home count = total scheme grant, and the all-in margin shown BOTH raw and WITH the grant applied side by side (‘18.2% → 21.4% with grant, +3.2 pts’) — so a reader can immediately see whether grant funding alone could close a viability gap. Pulled from the SAME engine as the Grant & Funding stage (grantEligibilityFor) so the numbers stay in sync, and always visible when affordable % > 0 (not only when a grant has been modelled). No engine-value change."},
  ...
  {v:"10.163", date:"Jul 2026", headline:"Fix: the Quick Appraisal's affordable-housing note no longer hard-codes KENT registered providers on every deal. It said the affordable homes ‘would be sold to a registered provider — confirm current appetite (in Kent, e.g. Golding Homes, Clarion, Moat, Southern Housing)’ and the RP research link searched ‘…Kent registered provider’ — stale copy that showed ‘Kent’ on an Essex (Maldon) site, or anywhere. Now location-neutral: the note points to ‘the RPs active in your area’ with nationally-operating examples (Clarion, Sanctuary, Places for People, L&Q), and the research link searches the deal's own town without a hard-coded county. No engine change. Flagged by a Maldon rebuild audit."},
  {v:"10.162", date:"Jul 2026", headline:"NEW ‘🔗 Fetch live rents (my endpoint)’ on Capitalisation — pull rents from a REAL data source you own, not just an AI estimate. A static web app can't hold an API key or fetch licensed / government data itself, so this posts the site (postcode, local authority, beds, houses-or-flats) to a data ENDPOINT you configure — a Google Apps Script web app, a Zapier zap, or any serverless function — and fills the 1/2/3/4-bed fields from its JSON reply, showing the source. What the endpoint looks up is YOUR choice and swappable without a redeploy: VOA / ONS official median rents (free, citable — the recommended baseline, same evidence class as the Land Registry sale-price call), a licensed rent API (postcode-level, paid), or an LLM with web search. Set the endpoint URL once (⚙ box, stored in the browser); the existing ‘🤖 AI: research & fill’ button stays as the zero-setup fallback. A ready-to-deploy Apps Script example implementing the exact request/response contract — with VOA / rent-API / web-search-agent options — ships in docs/rents-endpoint-example.gs. Uses a preflight-free POST so an Apps Script endpoint works with no extra CORS config. Nothing is sent until you click the button; rents stay editable and flagged to verify against a specific listing. No engine change."},
  {v:"10.161", date:"Jul 2026", headline:"Two board-proposal clarity tweaks so a NON-viable deal can't be skimmed as a green light. (1) The recommendation line no longer says a flat ‘Meets the 17.5% target margin on modelled assumptions’ when no land price is entered — because the model RESERVES the target profit and then solves for the land value, that phrasing read as ‘profitable’ even when the residual land value was NEGATIVE. It's now sign-aware: a positive residual reads ‘Reserves the 17.5% target margin and supports up to £Xm of land’; a nil/negative residual reads ‘Reserves the 17.5% target margin, but the residual land value is −£2.17m — the scheme does NOT support a land payment as modelled’. So a scheme that doesn't stack says so plainly. (2) The ‘Basis of figures’ build-cost line now states the rate is applied UNIFORMLY across house types (build £/sqft varies little by type at appraisal stage, unlike sale price which is type-specific; a QS cost plan prices each type) and that a named contractor quote in the source overrides the benchmark and appears in the assumptions log — pre-empting the ‘why doesn't unit size change the build rate?’ question honestly, rather than fabricating a per-type spread. No engine-value change — wording and disclosure."},
  {v:"10.160", date:"Jul 2026", headline:"Keystone now researches accurate CURRENT rents at BUILD time — so a deal comes out of the builder with realistic rents already fed into capitalisation, not the conservative table baseline you'd otherwise have to fix later on the Capitalisation stage. Keystone's ‘Complete with AI’ already researched per-house-type sale prices AND rents at build time, but it asked for ‘typical ACHIEVED’ figures (which read below what's actually advertised). It now uses the SAME accurate basis as the v10.159 Capitalisation rent research: current Rightmove / Zoopla ASKING rents for NEW-BUILD in the postcode, the new-build premium reflected, priced as HOUSES (a 3/4-bed house rents well above a flat of the same bed count), don't-round-down. So the rents fed into the capitalised / forward-fund exit are realistic from the first build — and the manual ‘AI: research & fill area rents’ button on Capitalisation (also v10.159) stays available to refresh them. Both still flagged to verify against live listings. No engine change — better rent inputs at build time."},
  {v:"10.159", date:"Jul 2026", headline:"Capitalisation rents now read closer to the real market — they were coming in LOW versus a live listings search. Two causes fixed. (1) The ‘AI: research & fill area rents’ prompt asked for ‘typical ACHIEVED’ rents (which skew below what's advertised) and didn't anchor on current listings — it now asks for the MEDIAN of CURRENT Rightmove / Zoopla ASKING rents for NEW-BUILD in the specific postcode, tells the model new-build commands a premium and not to round down, and is SCHEME-AWARE: for a houses (SFH) scheme it prices 3/4-bed HOUSES (which rent well above a flat of the same bed count) rather than generic ‘homes’. (2) When the rents are still the conservative MARKET-TABLE baseline (you haven't run the AI research or verified them), the page now shows an amber nudge saying so — the table baseline often reads below current new-build asking rents and understates the rental / forward-fund value — with a one-click prompt to research current figures. Rents stay fully editable and are always flagged to verify against live Rightmove / Zoopla. No engine change — better rent inputs and clearer provenance."},
  {v:"10.158", date:"Jul 2026", headline:"Three fixes from a Maldon forensic pass. (1) NEGATIVE residual land value now DISPLAYS on the Board Proposal instead of a blank em-dash. Three lines (the summary table's ‘Supportable residual land value’, the ‘how these figures were derived’ bullet, and the summary strip) suppressed a negative RLV to ‘—’ while it showed correctly elsewhere in the same report — a marginal/negative deal now reads ‘−£4.96m’ (in red) so the report is internally consistent. (2) STALE PROFIT TEXT: the ‘National Housebuilder’ buyer row hard-coded ‘~17.5% profit’ even when the deal's actual target is lower (e.g. 15%); it now shows the deal's real target profit %. (3) NEW PLANNING-CONSISTENCY cross-check on the Dashboard: the planning status, constraint verdict and risk level are set by separate AI passes that don't cross-validate, so they can disagree — e.g. Planning Status ‘Outline consent’ (a consent secured) while the Constraint Check rates the site AVOID and risk is only ‘Medium’. Rather than silently pick one, the Dashboard now flags the contradiction in red (‘⚠ Planning position looks inconsistent across stages’) and points to the Planning stage to reconcile, so a too-optimistic planning status can't quietly prop up the consented figures. No engine-value change — display, labelling and a consistency guardrail."},
  {v:"10.157", date:"Jul 2026", headline:"The appraisals now show the GRANT ELIGIBILITY ASSUMPTION even when no grant is modelled — reported: a scheme (Maldon) that doesn't stack was flagged AHP-eligible on the Grants page, but the appraisals were silent on grant, so you couldn't see whether public subsidy might close the gap. Previously grant only appeared on an appraisal once you'd entered a figure on the Grants stage (the default ‘developer margin’ treatment keeps grant OUT of the land value, so a marginal scheme showed nothing). Now, when no grant is modelled but the scheme carries meaningful affordable housing, the Quick Appraisal shows an amber ‘◐ Grant not modelled — may be AHP-eligible’ note, and the one-page / board appraisal's ‘Basis of figures’ carries the same, stating: the affordable-home count, the indicative AHP grant it implies (affordable homes × ~£45k/home, matching the Grants stage), that it's NOT in the figures, and — under the default treatment — how many margin points it would add (grant ÷ GDV) so you can see it can turn a marginal scheme viable. Clearly indicative: AHP is area/tenure-specific and needs a Registered Provider partner + Homes England sign-off; model it properly on the Grants stage. No engine-value change — disclosure only; the residual is unchanged."},
  ...
  {v:"10.152", date:"Jul 2026", headline:"Consistency & clarity sweep across the SFH journey, from a deep forensic pass. (1) The SFH House Mix ‘Exit / buyer allocation’ summary no longer contradicts the rest of the tool: when every house-type row is left as ‘Private retail sale’, the engine actually prices the scheme on the Tenure Mix split (per-row tenure > Tenure Mix > scheme ah%), but the summary used to roll up only the per-row tenure and report 100% private — flatly contradicting the affordable split the RLV was built on. It now shows the Tenure Mix allocation (the split the valuation used), badged as such. (2) NEW Propagation Audit check compares the affordable proportion across House Mix per-row tenure, the Tenure Mix stage and the Planning ah% — honouring the engine's precedence — so a per-row tenure silently overriding a Tenure Mix split is no longer invisible to the audit. (3) The two profit figures on the House Mix page (the 17.5%-target residual vs the fuller Net Profit after SDLT-on-land & selling costs) now carry a note explaining they answer different questions — one SETS profit to solve for the land price, the other MEASURES it after real transaction costs — so they no longer look like a bug. (4) Financial Modelling: the build line now states that its construction £/sqft + fees + contingency is the SAME cost as the all-in £/sqft shown on House Mix/Dashboard (£175 × 1.17 ≈ £205), footing to the same total; and the Commercial Analysis ‘Cassidy Dev Profit’ caption now shows the deal's ACTUAL profit % instead of a hardcoded ‘20% target on GDV’. (5) The Capitalisation Multi-Route Exit table now flags loss-making / thin-margin tenure rows (e.g. Social Rent at a negative margin) with a warning badge and an explanation that deep-discount affordable is cross-subsidised by the open-market homes — instead of presenting a −18% row as plain data. No engine-value change; all display, disclosure and audit-coverage. (Two larger items from the same pass — splitting bulk vs retail sales pacing for a realistic IRR, and auto-populating the Exit Strategy dropdowns — are booked as follow-ups.)"},
  {v:"10.151", date:"Jul 2026", headline:"Keystone now WORKS OUT an approximate postcode from the place name when the source doesn't state one — so ‘land north of Staplehurst’ resolves to the right local market instead of falling back to national averages. When no postcode is written but a town / village / place is named, the AI extraction now infers the postcode DISTRICT (outcode, e.g. TN12) from the location and flags it as inferred. The ‘Confirm the postcode’ panel (v10.150) gains a third, AMBER state for this: ‘Postcode TN12 was INFERRED from Staplehurst — confirm it's right (resolves to Tunbridge Wells)’, with a one-click ✓ Confirm button and the field editable if the guess is wrong (editing marks it confirmed). Green = a stated postcode that resolves; amber = an inferred one to check; red = nothing resolvable (still gated). The inferred outcode is enough to pick the correct pricing market, and because the deal now resolves, the build no longer blocks on national averages — but you always see and confirm the guess first. Requested after the postcode-accuracy work: an approximate postcode can be worked out from indicators like the place name. No engine-value change."},
  {v:"10.150", date:"Jul 2026", headline:"Keystone now makes you CONFIRM THE POSTCODE before building — location can no longer be missed into a national-average valuation. AI brief extraction is non-deterministic: the exact same source document yielded a postcode on one run and a blank on the next, and a blank silently priced the deal on UK NATIONAL AVERAGES (≈£300/sqft sale, £215/sqft build, 4.75% yield) instead of the correct local figures — which swung a headline residual land value by ~33% (£163m → £109m) on identical re-runs. Since postcode/location is the single biggest driver of the valuation, the Build card now shows a ‘Confirm the postcode’ panel: it displays the extracted postcode, lets you correct it inline (written straight back into the brief), and turns RED when the location doesn't resolve to a local market (green, naming the market, when it does). And ‘Build deal & load it’ now GATES on it — if the location can't be resolved and no postcode was extracted, it blocks with a clear warning (‘this can swing the land value by tens of millions’) and makes you either add the postcode or explicitly choose to build on national averages. No engine-value change; this stops a missed postcode from silently producing a wrong headline valuation. Flagged by a from-scratch rebuild audit of the Staplehurst deal."},
  {v:"10.149", date:"Jul 2026", headline:"Deal Dashboard now flags the BEFORE-LAND position when no land price is entered — the same fix the Financial Modelling stage already had (v10.136). With no land price set, ‘Total Dev Cost’ excludes land (it's £0), so the Dashboard's ‘Profit on Cost’ and ‘Margin on GDV’ tiles were actually showing GDV minus build/fees/finance/S106 only — i.e. LAND + PROFIT combined (e.g. £301m / 38.1%), which reads far above the 17.5% target the appraisal is built on and could mislead anyone comparing the two. When no land price is entered, the two tiles now relabel to ‘Surplus before land’ and ‘Margin before land’ (amber, sub-labelled ‘this is LAND + profit, not profit alone’ / ‘not the 17.5% target margin’), and a note explains the surplus splits into the residual land value plus the target profit — so at that land value the developer margin IS the 17.5% target. The reconciliation identity (surplus = RLV + target profit) is exact. Once a guide price is entered the tiles show the real ACTUAL margin as before. No engine-value change; labelling and disclosure only. Flagged by a forensic audit of the rebuilt Staplehurst deal."},
  {v:"10.148", date:"Jul 2026", headline:"Accessibility: the Reports hub cards are now real <button>s, not clickable <div>s. They worked for a mouse, but keyboard users, screen readers and automated tooling couldn't see them as actionable controls — they're now proper buttons (Tab-focusable, Enter/Space activate, an aria-label announcing ‘Open [document]’), with the native button styling reset so they look identical to before. Purely an accessibility / automation fix; no behaviour or figure change. Flagged in a live check of v10.147. (The repeated Chrome async-listener errors in the console are browser-extension noise, not Landform — they don't touch report generation.)"},
  {v:"10.147", date:"Jul 2026", headline:"Six fixes from a forensic pass on the rebuilt Staplehurst deal. (1) VIABILITY SCENARIO now needs a guide price: with none entered it showed ‘to a 15% margin → a LOWER sale price’, which read backwards (cutting the sale price looked like a route to viability) — page 2 now shows a clear ‘enter a guide price to solve this scenario’ prompt instead of the misleading lever table, and only solves once a land price exists. (2) SALE £/sqft RECONCILED: the viability page derived a GDV-blended £/sqft (~£416, which folds in the affordable discount) while the one-pager showed the open-market £444 — the scenario now uses the same open-market SF.basePsf, so both read £444. (3) EXIT ‘Sell Now (land)’ no longer mislabels an unconsented site: it read ‘£146.87m · immediate exit, no build risk’ (the consented figure) — on a promotion site it now leads with today's pre-consent value and states the consented land value is only realisable AT consent (a forward sale), not today. (4) MARKET BENCHMARK honesty: the Land Valuation ‘Market data: Tunbridge Wells’ banner now says Tunbridge Wells is the NEAREST market with data when the site is elsewhere (e.g. Staplehurst TN12), names the real locality, and prompts local comps + confirming the LPA separately. (5) NORMALISE PROBABILITIES: a one-click ‘⚠ Normalise to 100%’ button on the planning-scenario table rescales the probabilities proportionally (largest-remainder rounding to exactly 100) — the expected value was already normalised, but a table visibly summing to 80% read as uncalibrated. (6) FINANCE LABELS: Financial Modelling showed three finance numbers on three different debt bases (engine S-curve peak-debt in the appraisal, a crude ‘held across the whole programme’ upper estimate, and the phased-cashflow interest); a new note names which is which and says to use the engine figure for the appraisal, with the other two bracketing it. No engine-value change — guardrails, reconciliation and labelling only."},
  ...
  {v:"10.140", date:"Jul 2026", headline:"Board Proposal external review — batch 2. (1) Rental reconciliation (#9): the report showed a generic ‘Market BTR rent’ (an apartment benchmark, e.g. £980/mo) next to an NOI built from the scheme's own higher HOUSE rents — so the implied net rent per home read ABOVE the ‘gross’ benchmark, which is impossible. It now labels that row ‘Market BTR benchmark (apartments — a low proxy for houses)’ and adds a ‘Scheme rent per home (drives NOI)’ row showing gross · net after ~25% costs, so gross ≥ net always reconciles. (2) Density upside (#3): the Keystone capacity banner applied ~20 homes/acre to the GROSS site (e.g. 271.8ac × 20 = 5,436 — not credible), when ~20/acre is a NET-developable density. It now applies it to the net-developable area (~60% of gross after roads, open space, SuDS, BNG & schools), so a 271.8-acre site reads ~3,260 homes, and the ‘model this’ button sets the matching gross density. (3) Pension yield wording (#12): the pension/sovereign long-income yield was fixed at 4.5% and called ‘tighter’ even when the deal's BTR yield was 4.3% (4.5% is wider). It's now derived ~30bps tighter than the deal yield, so it's genuinely tighter. Note on the affordable-% sensitivity (#6): the tool already detects when the scheme-level affordable % is INERT (because affordable is captured as per-row tenure) and hides it as a lever — so a ‘30%→20% = no change’ is that precedence, not a calc error; a clarifying message is the remaining refinement, flagged for confirmation. 578 tests pass."},
  {v:"10.139", date:"Jul 2026", headline:"Board Proposal fixes from a full external review (batch 1 of 2). (1) The most serious: the board paper was showing a ‘38.3% developer margin’ / ‘Strong margin’ — that figure is the pre-land surplus (profit + residual land value, ÷ GDV), NOT the developer’s margin. With no land/guide price entered it read ~3× the true return. The report now shows the HONEST margin: the 17.5% target margin at the residual land value (or the achieved margin at an entered guide price), across the headline KPI, the appraisal table, the ‘how derived’ note and the verdict line (which now reads ‘Meets the 17.5% target margin on modelled assumptions’ instead of ‘Strong margin’). (2) The 65% LTV refinance figure was labelled ‘equity released’ — it is GROSS loan proceeds; now labelled as such, with a note that net equity is after debt repaid, rolled-up interest, fees and debt-service cover. (3) The multi-buyer value range now carries a note that a completed investment value, a residual land bid and an affordable-package price are NOT like-for-like, and the decision metric is net profit / NPV / net equity, not the highest gross figure. Batch 2 (affordable-% sensitivity, rental NOI reconciliation, the gross-acre density ‘upside’) to follow. 578 tests pass."},
  {v:"10.138", date:"Jul 2026", headline:"The header now shows the SCHEME NAME, so you always know which deal you're in. The fixed top bar showed only the current stage (e.g. ‘Financial Modelling’). It now shows the scheme name in bold with the stage beneath it — the name comes from the Dashboard ‘Deal name’ field (or the site address if you haven’t named it), so switching between schemes or picking a deal back up is unambiguous. Purely a display addition; no calculation change."},
  {v:"10.137", date:"Jul 2026", headline:"Follow-up consistency fixes: SFH sensitivity anchored to its own headline, and two ‘18%’ mislabels corrected to 17.5%. (1) The Sensitivity Analysis on the SFH House Mix page showed a ‘Base’ residual land value a few £m below the RLV headline directly above it — because the scenario table recomputed the residual with a simplified flat-finance formula that drifted from the headline’s S-curve finance. The Base now reads exactly the headline RLV, with the pessimistic/optimistic cases shown as the modelled swing from it (same anchoring already used on the Land Valuation stage). (2) Two spots rounded the 17.5% profit target to ‘18%’ — the Land Appraisal ‘target margin’ chip and the Capitalisation deal-evolution note — both now read 17.5%. No engine change; 578 tests pass. (Note: a separate, deeper item remains for a decision — the SFH House Mix / Detailed Appraisal compute the residual WITHOUT the affordable grant while the RLV / Quick Appraisal engine includes it, and the Detailed Appraisal runs its own land figure and margins; reconciling those is a modelling-treatment choice, flagged rather than silently changed.)"},
  {v:"10.136", date:"Jul 2026", headline:"Financial Modelling no longer looks 3× too profitable when no land price is entered. With no land price set, the Financial Modelling stage was showing Profit and Margin BEFORE the cost of buying the land — e.g. ‘Profit £303m, Margin 38.3%, ✓ Viable’ on a scheme whose true post-land margin is ~12–17%. That figure is really the residual land value PLUS developer profit (the pot to split between land and developer), not the scheme’s return — read in isolation it made the deal look roughly 3× more profitable than it is. Now, when no land price is entered, the stage shows an amber ‘⚠ No land price entered’ banner, relabels the tiles ‘Profit (before land)’ / ‘Margin (before land)’, and replaces the green ‘✓ Viable’ badge with ‘◐ Before land — this is the pre-land margin, not the scheme’s viability’, pointing to the SFH House Mix / Dashboard for the true post-land figure. Enter a land price and it reads normally. No engine change; 578 tests pass."},
  {v:"10.135", date:"Jul 2026", headline:"Quick Appraisal now shows the affordable grant consistently — the ‘two different RLVs’ on that page were WITH-grant vs WITHOUT-grant. The headline Residual Land Value comes from the engine, which adds the Homes England affordable grant to the residual (v10.126). But the ‘Target profit → what you can pay for the land’ widget, its profit ladder, and the cost-breakdown total all left the grant OUT — so on an affordable scheme they sat the whole grant below the headline (for a 1,800-home, 35%-affordable scheme that’s ~£50m: e.g. a headline of £138m next to a widget showing £88m). That read as a bug/‘stale figure’, but both were ‘right’ — they just differed by the grant. The widget and ladder now include the grant so they match the headline, and the cost breakdown shows an explicit ‘+ Affordable grant (AHP)’ credit line so it foots and you can see exactly how much of the residual is grant-dependent (needs a Registered Provider partner + Homes England — indicative). Verified: with grant, headline = widget = breakdown to the penny. 578 tests pass."},
  {v:"10.134", date:"Jul 2026", headline:"Rename a deal in-app — a Deal name field on the Dashboard. Naming a deal used to happen only through the browser's native Save prompt, with no way to rename afterwards (and that prompt is clunky on an iPad and can't be used at all by an automated browser). The Dashboard now has a ‘Deal name’ field you can edit any time; Save uses it as the name, and because re-saving an already-saved deal updates the same portfolio card, editing the field IS the rename. Leave it blank and it still falls back to the site address. No calculation change."},
  {v:"10.133", date:"Jul 2026", headline:"The SFH House Mix now flags — and one-click fixes — a sale price that's adrift from the tool's own Land Registry estimate. The sale £/sqft is the single biggest lever in the whole appraisal, and there was a silent trap: when Keystone can't resolve a location it falls back to a generic £300/sqft, while the SFH page separately shows a location-specific new-build estimate (Land Registry existing value + new-build premium, e.g. £445) that was only ever displayed in the field's label, never applied. So a deal could run ~30% below the tool's own evidence with nothing flagging it. Now, whenever the stored Base Sale £/sqft differs from the Land Registry new-build estimate by 8%+, a banner shows the gap (‘Sale price is 33% below the Land Registry estimate’) with a one-click ‘Apply £445/sqft’ that adopts the estimate AND re-prices every house type off it. Still flagged to verify against a real local comp — it's an estimate, not gospel. Surfaced by an end-to-end test where a Staplehurst deal ran on £300 vs the tool's £445. 578 tests pass."},
  {v:"10.132", date:"Jul 2026", headline:"Two Keystone fixes from the end-to-end test: a realistic finance default and a planning-authority guardrail. (1) Keystone was defaulting the development-finance rate to 12% — above the 7–9% UK senior-debt benchmark, inconsistent with the rest of the tool (Financial Modelling 7.5%, Detailed Appraisal 8%), and DOUBLE-conservative because the engine already models finance on a peak-debt S-curve. It overstated finance by tens of £m and understated the residual. The default is now 8% (the benchmark midpoint); a rate quoted in the brief still wins. (2) A postcode’s POSTAL TOWN often isn’t the same as its planning authority — e.g. a TN postcode (Tunbridge Wells) can sit in Maidstone Borough, which has different affordable-housing, S106 and CIL policy. When Keystone resolves a location from its postcode it now explicitly warns to CONFIRM the Local Planning Authority separately, so a site isn’t silently benchmarked against the wrong council. 578 tests pass."},
  {v:"10.131", date:"Jul 2026", headline:"Detailed Appraisal now explains its £0 fee/contingency lines instead of looking like an omission. When the deal's build £/sqft is ALL-IN, the Detailed Appraisal correctly zeroes professional fees, contingency, infrastructure and CIL (they're already inside the build rate, so adding them again would double-count). But those £0s read as a mistake — an end-to-end review flagged them as ‘~£57m of fees & contingency missing’, which would be the WRONG thing to add back on top of an all-in rate (it would double-count and understate the margin). A green banner now makes it explicit: these lines are £0 because they're covered by the all-in build rate, not because they're omitted — switch to a construction-only build £/sqft on the SFH stage to itemise them. Purely explanatory; no figure changes. 578 tests pass."},
  {v:"10.130", date:"Jul 2026", headline:"Simpler navigation — the six report stages are now one ‘Reports’ hub, and Executive Summary is retired. The sidebar’s Report group had SIX separate output stages (Site Scorecard, Investor Memorandum, Data Room, Stakeholder Suite, Executive Summary, Board Proposal) — a lot of clutter for what are all output documents built from the same reconciled figures. They’re now behind ONE ‘📑 Reports’ hub: a single sidebar entry that opens a page of cards, one per document, so you pick the one you need. Every document still renders exactly as before (and stays directly linkable — e.g. the Quick Appraisal’s ‘board paper →’). ‘Executive Summary’ is RETIRED: its completion tracker just duplicated the Deal Dashboard and its written brief is superseded by the Board Proposal’s new Manager Summary; its old link now lands on the hub. No engine or figure change — purely navigation. 578 tests pass."},
  {v:"10.129", date:"Jul 2026", headline:"The top bar is now a FIXED header — the navigation stays reachable wherever you scroll. The header (hamburger menu, stage title, and the action buttons) was set to ‘sticky’, but the app’s ancestors all use overflow-x:hidden to stop sideways scrolling on mobile — and that silently breaks position:sticky (sticky is also unreliable on older iOS), so the bar scrolled off the top and you had to scroll all the way back up to change stage. It’s now position:fixed — pinned to the top of the screen at all times, viewport-relative so the overflow rules can’t dislodge it — with the page content padded down so nothing hides underneath. On a phone the ☰ menu is always one tap away; on desktop the sidebar was already fixed. No calculation change."},
  {v:"10.128", date:"Jul 2026", headline:"NEW ‘🧭 Manager summary’ — one concrete evaluation report that pulls the whole appraisal together for managers. On the Board Proposal stage, next to the one-pager and board paper, a new button generates a single, sendable Manager Summary Appraisal that answers ‘can a manager evaluate this with confidence?’. It leads with an engine-derived VERDICT (Proceed / Caution / Does-not-stack), the reconciled headline figures (GDV, the full cost stack, residual land value + the exit-route range — all from the one engine, so they match every other screen), and — the key part for trust — a CONFIDENCE table that tags each key input Verified / AI-researched / Assumption (sale £/sqft, build cost, affordable %, rents & yield, finance, S106, planning status, programme), so a manager instantly sees what’s evidence-backed versus a working assumption. Everything tagged ‘Assumption’ is rolled up into a ‘Resolve before an offer’ checklist. On generate it AUTO-RUNS two AI passes — a benchmark/accuracy sense-check against the major housebuilders (Persimmon/Barratt/Taylor Wimpey/Bellway) and BCIS, and a risk / red-flag review — and prints them in the report. Opens as a web page to read on screen and Print / Save as PDF to send to management. No engine change; 578 tests pass and the report generates clean (structure + no-leak verified)."},
  ...
  {v:"10.96", date:"Jul 2026", headline:"New ‘🌳 Landowner pack’ — the proposal that wins the land. It leads with the LANDOWNER's upside (their land's value today in current use vs its value with consent, and the uplift), then offers two fair, de-risked structures side by side (an option agreement, and a promotion agreement where Cassidy funds and carries the planning at its own risk and the owner pays nothing and keeps the land until consent), a ‘why Cassidy’ credibility section and the owner's protections (overage, longstops, tax). Reuses the Land stage's option/promotion inputs. Opens from the Investor Suite → Outreach Kit, and is now the first page of the ‘Generate ALL packs’ document. Landform now covers the whole deal — from securing the land to funding, delivery and consent."},
  {v:"10.95", date:"Jul 2026", headline:"‘📚 Generate ALL packs’ — one click produces the whole stakeholder set (investor teaser + housing-association/RP + lender + local-authority) as a single printable document, each pack on its own page. All four run off the same engine, so the figures reconcile across every audience. In the Investor Suite → Outreach Kit, alongside the individual pack buttons."},
  {v:"10.94", date:"Jul 2026", headline:"New ‘🏛️ Council pack’ — a Planning Benefits Statement for the local authority, making the positive case for consent: housing delivery (market + affordable) toward the authority's requirement and five-year land supply, the affordable tenure split, S106 / CIL contributions (education, highways, health, open space, community), 10% Biodiversity Net Gain, design & sustainability (NDSS, EPC B, Future Homes, SuDS), and the economic benefits — construction jobs (~1.5/home), construction investment and ongoing council-tax revenue. Opens from the Investor Suite → Outreach Kit. Landform now produces a tailored pack for every stakeholder: investor, housing-association partner, financier AND local authority."},
  {v:"10.93", date:"Jul 2026", headline:"New ‘🏦 Lender pack’ completes the stakeholder set — a development-finance request built for a senior lender: the indicative facility (sized on typical caps — ≤60% loan-to-GDV, ≤90% loan-to-cost), the developer equity required, peak debt on the S-curve, day-1 land advance, cover & leverage (GDV cover, margin buffer, rolled-up interest), the exit that repays the loan, a downside stress showing loan-to-GDV under GDV falls of 10–20%, and the security package. Opens from the Investor Suite → Outreach Kit. Cassidy now has a tailored pack for every stakeholder — investor, housing-association partner and financier."},
...
function basisOfFigures(data){
  ...
  lines.push({ k:"Build cost", v:"£" + buildPsf + "/sqft" + (M.buildInclusive
    ...
    : " (construction only; professional fees, contingency, roads & SuDS are added as separate lines).") + " BCIS-range benchmark for the scheme type — confirm with a QS cost plan. Set deliberately on the CAUTIOUS side: a national housebuilder often builds cheaper, so a keener build rate would INCREASE the residual land value — this is a floor, not a stretch. Applied UNIFORMLY across house types: build £/sqft varies little by type at appraisal stage (unlike sale price, which is type-specific), so a single all-in scheme rate is used — a QS cost plan prices each type. Any contractor quote in the source (e.g. a named build £/sqft) overrides the benchmark and is shown in the assumptions log."
  ...
  lines.push({ k:"Floor areas (GIA)", v:"Enter each home's floor area as HABITABLE Gross Internal Area (GIA) — the internal area to the inside face of the external walls (RICS Code of Measuring Practice), the same figure an agent markets, EXCLUDING the garage. GIA counts internal walls, hall/landings and the stairs on every floor. The sale £/sqft and the build £/sqft are applied to this SAME GIA, so they are like-for-like (houses are both marketed AND costed on GIA). NOT included / handled separately: the garage, drive, car port, detached stores and other EXTERNAL WORKS — a garage is a cheap shell (~£600–1,000/m², vs ~£1,800–2,500/m² habitable) and is valued as a lump sum, not at the house £/sqft. Your all-in build rate is intended to absorb these external works; if you switch to a construction-only rate, add garages/externals separately. Apartments differ: sales/rents move to NET INTERNAL AREA (NIA ≈ 80–85% of GIA) while build stays on GIA — houses are ~95–100% (no communal areas), which is why they use GIA throughout." });
  ...
  if(num(M.grantIncome) > 0){
    ...
    // v10.157 — when NO grant is modelled, still state the grant ELIGIBILITY ASSUMPTION so a reader
    // sees whether public subsidy could improve a marginal scheme (reported: a non-stacking deal was
    // flagged eligible on the Grants stage but the appraisal was silent on grant).
    var _ge = (typeof grantEligibilityFor === "function") ? grantEligibilityFor(data) : null;
    if(_ge){
      var _gv;
      if(_ge.eligible){
        _gv = "None modelled in this appraisal. With " + _ge.affordableHomes + " affordable homes the scheme is indicatively AHP-eligible — around " + fmt(_ge.indicativeAhp) + " (~£" + Math.round(_ge.perHome/1000) + "k/home). Under the default 'developer margin' treatment that is upside to the margin (roughly +" + (Math.round(_ge.marginUpliftPts*10)/10) + " pts on GDV), NOT capitalised into the land price — so it does not lift the residual, but it can turn a marginal scheme viable. Model it on the Grants stage (needs a Registered Provider partner + Homes England sign-off). Indicative only.";
      } else {
        _gv = "No affordable-housing grant is modelled, and the scheme has too few affordable homes to be a meaningful AHP candidate. If the tenure mix changes, check the Grants stage.";
      }
      lines.push({ k:"Affordable grant", v:_gv });
    }
  }
...
function dealExit(data){
  ...
  return {
    ...
    // Phased sale sits on the plot-sales basis but is a distinct route — label it so the
    // reports match the "Phased Exit" wording on the Exit Strategy dropdown rather than the
    // generic "open-market plot sales" (which read as an inconsistency to reviewers).
...
// about grant — so you couldn't see whether grant might close the gap. This surfaces one honest
// figure the appraisal can print: is the scheme (indicatively) AHP-eligible, how much grant that
// implies (affordable homes × the per-home rate — matching the Grants stage's ~£45k default), and
// whether it's already modelled in this appraisal (and under which treatment). Indicative only —
// AHP is area/tenure-specific and needs an RP partner + Homes England sign-off.
function grantEligibilityFor(data){
  data = data || {};
  var M = (typeof computeSFHMetrics === "function") ? computeSFHMetrics(data) : {};
  var g = (data && data.grants) || {};
  var affordableHomes = Math.round(num(M.affordableHomes) || 0);
  var appliedGrant = num(M.grantIncome) || 0;                    // grant currently in the appraisal
  var treatment = M.grantMode || grantTreatmentMode(data) || "margin";
  var perHome = num(g.grantPerAffHome) || 45000;                 // AHP indicative (Grants stage default)
  var indicativeAhp = affordableHomes > 0 ? affordableHomes * perHome : 0;
  var eligible = affordableHomes >= 10;                          // meaningful affordable delivery + an RP
  var gdv = num(M.gdv) || 0;
  var marginUpliftPts = gdv > 0 ? (indicativeAhp / gdv) * 100 : 0;   // grant as % of GDV ≈ margin points added
  return {
    affordableHomes: affordableHomes, appliedGrant: appliedGrant, applied: appliedGrant > 0,
    treatment: treatment, perHome: perHome, indicativeAhp: indicativeAhp, eligible: eligible,
    marginUpliftPts: marginUpliftPts
  };
}
...
var SHARED_FIELD_GROUPS = [
  ...
  // so editing the yield there changed nothing (the engine's dealYield reads capitalise.targetYield)
  // and didn't sync to Capitalisation / Financial Modelling. Now wired both ways.
  [["capitalise","targetYield"],["fin","exitYield"],["exit","exitYield"]],
