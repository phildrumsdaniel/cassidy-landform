...
function renderIM(at, city, data, gdv, lc, up, user){
    ...
    function imAppendixHTML(){
      ...
      var prog=TL?('<h2>Programme &amp; Timeline</h2><p style="font-size:11px;color:#6A6F97">Two separate clocks — winning planning consent (councils routinely exceed the 13-week statutory target on major/strategic applications) and building out. Current position: '+TL.statusLabel+'. A forward-fund only starts once the scheme is consented and fundable.</p>'+
        '<table style="width:100%;border-collapse:collapse;font-size:12px">'+
        trow("Planning to consent","~"+TL.planningYears+" yrs ("+TL.planningMonths+" months)")+
        trow("Build-out programme (incl. sales runoff)","~"+TL.buildYears+" yrs")+
        trow("Total money-in to exit","~"+TL.totalYears+" yrs",true)+
        '</table>'):'';
      return fin+prog+sens+ff+dr;
    }
    function printIM(){
      var imData=data.im||{};
