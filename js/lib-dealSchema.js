  ...
  { key:"planning", label:"Planning strategy & risk",
    ...
    prompt:function(data){ var c=_kjContext(data);
      return "Assess planning for a "+c.units+"-home residential scheme in "+c.town+(c.lpa?" (LPA "+c.lpa+")":"")+", current planning status: "+(c.status||"unallocated/none")+
        ". Output EXACTLY this JSON: {\"riskLevel\":\"low|medium|high\",\"bng\":\"on_site|off_site|exempt\",\"gateway\":\"na\",\"planningProb\":<0-100 probability of achieving consent>,\"timelineMonths\":<realistic months to a consent, grounded in UK 2024-26 practice: councils routinely miss the 13-week major target — only ~1 in 5 majors is decided in time, the rest run on Extensions of Time — so a major residential decision is typically 7-12+ months; a large/strategic site is 24-36 months to a resolution to grant with S106; an UNCONSENTED site promoted through the local plan is 3-7 years (36-84 months). Do NOT quote the 8/13/16-week statutory periods>,\"summary\":\"2-3 sentence planning strategy incl. the route (allocation/outline/full) and key policy risks\"}. Houses under 11m tall ⇒ gateway \"na\"."; },
