...
var KEYSTONE_JOURNEY_FILLERS = [
  ...
  // rather than left at zero. Desktop judgement, to be verified on site.
  { key:"site", label:"Site appraisal — location, transport, ground, tenure",
    ...
    prompt:function(data){ var c=_kjContext(data);
      return "Desktop site appraisal for a "+c.units+"-home residential scheme"+(c.address?" at "+c.address:"")+" ("+(c.postcode||"postcode unknown")+") in "+c.town+", ~"+c.acres+" acres. Judge each factor from what a UK land buyer would infer from the location. Output EXACTLY this JSON: {\"proximity\":\"excellent|good|fair|poor\",\"transport\":\"excellent|good|fair|poor\",\"contamination\":\"clean|minor|unknown|major\",\"tenure\":\"freehold|long_leasehold|short_leasehold\",\"constraint\":\"none|minor|moderate|major\",\"summary\":\"1-2 sentences justifying the ratings\"}. proximity = walk/cycle access to shops, schools & employment. transport = train/bus/cycle connectivity. contamination = likely ground condition given prior use (greenfield ⇒ clean; unknown prior use ⇒ unknown). tenure = most likely land tenure (assume freehold unless clearly otherwise). constraint = planning-constraint severity (none/outline; TPO/listed adjacent; conservation/Flood Zone 2; Green Belt/Flood Zone 3)."; },
