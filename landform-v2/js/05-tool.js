...
var JOURNEYS = {
  ...
  // ── DASHBOARD ──────────────────────────────────────────────────────────────
  // ── PLANNING MONITOR ──────────────────────────────────────────────────────
  function renderPlanningMonitor(){
  ...
  function renderIM(){
    ...
    function generateSection(sec){
      var prompts={
        exec:"Write the executive summary section (150 words) for an investor memorandum. Site: "+addr+", "+cityDisp+". GDV: "+fmt(gdv)+". RLV: "+fmt(num(rlvD.rlv||lc))+". Margin: "+pct(num(f.marginPct||0))+". Units: "+units2+". Planning: "+planStatus+", LPA: "+lpa+". Lead with the investment opportunity, headline numbers, and recommended action. Professional UK property investment tone.",
        ...
        e("div",{style:{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}},
          ...
          e("button",{
            onClick:function(){
              ...
              '</style></head><body>'+
              '<div class="header"><div class="h-logo">CASSIDY GROUP</div><div class="h-title">Grant Strategy Pack</div><div class="h-title" style="font-size:16px">'+addr+'</div><div class="h-sub">'+cityName(cityVal)+(lpa?" · "+lpa:"")+" · Prepared "+new Date().toLocaleDateString("en-GB")+'</div></div>'+
  ...
  // ══════════════════════════════════════════════════════════════════════════
  function renderProcessNavigator(){
...
e("span",{style:{color:"#9A7B3E"}},
          ...
          e(Inp,{label:"Sale £/sqft"+(pcData&&pcData.salePsf?" — auto from postcode: £"+pcData.salePsf:m?" — auto from "+cityName(city)+": £"+Math.round(m.btr*8.5/12):""),type:"number",value:ep.salePsf,onChange:function(v){up("epe","salePsf",v);},placeholder:"£"+Math.round(salePsf)+" /sqft"}),
...
e("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}},
  ...
  function renderStage(){
    if(stage==="portfolio")return renderPortfolio();
    if(stage==="scorecard")return renderScorecard();
    if(stage==="placona")return renderPlacona();
    if(stage==="navigator")return renderProcessNavigator();
    ...
    if(stage==="capitalise")return renderCapitalise();
    if(stage==="grants")return renderGrants();
    if(stage==="teaser")return renderTeaser();
    if(stage==="im")return renderIM();
    if(stage==="dataroom")return renderDataRoom();
    if(stage==="viability")return renderViability();
    if(stage==="meetings")return renderMeetings(data, up, user);
    if(stage==="monitor")return renderPlanningMonitor();
    ...
    if(stage==="epeworkflow")return renderEPEWorkflow();
    if(stage==="landworkflow")return renderLandWorkflow();
    if(stage==="recovery")return renderRecovery();
    if(stage==="summary")return renderSummary();
    if(stage==="flowcharts")return renderFlowcharts();
    if(stage==="epe")return renderEPE();
    if(stage==="rlv")return renderRLV();
    if(stage==="sfh")return renderSFH();
    if(stage==="hra")return renderHRA();
    if(stage==="land")return renderLand();
    if(stage==="planning")return renderPlanning();
    if(stage==="fin")return renderFin();
    if(stage==="scraper")return renderScraper();
    if(stage==="dd")return renderDD(data, setData, user, up);
    if(stage==="risks")return renderRisks(at, data, setData, up, user);
    if(stage==="exit")return renderExit();
